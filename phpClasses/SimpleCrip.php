...
if (!class_exists('mavalerio\phpClasses\SimpleCrip')) {
class SimpleCrip{
    ...
    public function decrypt($ivHashCiphertext, $password, $inputBinary=false, $mode='r') {
        ...
        //echo $metodoCompressao. $metodoCripto;

        $this->set($this->compressoes[$metodoCompressao]);
        if($metodoCripto == 'r'){
            $uncript = $this->richDecrypt($ivHashCiphertext, $password, $inputBinary);
        }else if($metodoCripto == 'p'){
            $uncript = $this->poorDecrypt($ivHashCiphertext, $password, $inputBinary);
        }else{
            echo "Compactado com $metodoCompressao em criptografado com $metodoCripto";
            debug('FALHA');
        }
    ...
    private function richDecrypt($ivHashCiphertext, $password, $inputBinary=false) {
        if ($this->DBG) echo "Decriptacao RICH : $ivHashCiphertext<br>\n";
        if ($this->DBG) echo "AESKEY: $password <br>\n";
        ...
        if ($this->DBG) echo "KEY: ".\base64_encode($key). " <br>\n $key<br>\n";
        if ($this->DBG) echo "IV : " . \base64_encode($iv) . " <br>\n $iv<br>\n";
        if ($this->DBG) echo "HASH: $hash <br>\n";
        ...
        if ($this->DBG) echo "DADO:<pre>" . print_r($uncript, 1) . "</pre><br> \n";
        //echo $uncript;
        return $uncript;
    }
    ...
    public function compress($data){
        if($this->DBG) echo "Comprimir usando: $this->compressMethod. <br>\n";
        ...
        if ($this->DBG) echo "<pre>".print_r($compress,1)."</pre><br> \n";
        return $compress;
    }
    private function uncompress($data){
        error_clear_last();
        if ($this->DBG) echo "Descomprimir usando: $this->compressMethod. <br>\n";
        if(is_null($data)) return '';
        if($data == '') return '';
        if($data == '""') return '';
        if($data == "''") return '';
        ...
        if ($this->DBG) echo "<pre>".print_r($uncompress,1)."</pre><br> \n";
        return $uncompress;
    }
